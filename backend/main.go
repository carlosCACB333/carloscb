...
func main() {
	...
	jwtManager := libs.NewJWTManager(cfg.JWTSecret, cfg.AccessTokenDuration)
	go runGatewayServer(cfg, db, taskDistributor, jwtManager)
	go runTaskProcessor(cfg, db, &redisOpts)
	runGrpcServer(cfg, db, taskDistributor, jwtManager)
...
func runGrpcServer(cfg *utils.Config, db *gorm.DB, distributor worker.TaskDistributor, jwtManager *libs.JWTManager) {
	...
	userServer := servers.NewUserServer(cfg, db)

	interceptor := mdw.NewInterceptor(jwtManager)
...
func runGatewayServer(cfg *utils.Config, db *gorm.DB, distributor worker.TaskDistributor, jwtManager *libs.JWTManager) {
	...
	if err := http.Serve(listener, mdw.HttpLogger(mux)); err != nil {
