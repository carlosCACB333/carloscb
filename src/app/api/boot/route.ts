...
import { createChatpdfMessage } from '@/grpc/chatpdf-messages';
import { getContext } from '@/grpc/chatpdf';
...
export async function POST(req: Request) {
  ...
  const stream = OpenAIStream(response, {
    onStart: async () => {
      // save user message
      await createChatpdfMessage(chatId, userMsg, "user")
    },
    onCompletion: async (message) => {
      // save IA message
      await createChatpdfMessage(chatId, message, "assistant")
    }
  });
