...
import {
  ChatpdfMessage,
  CreateChatpdfMessageReq,
} from "@/pb/chatpdf-messages_pb";
import { GenericReq, GenericRes } from "@/pb/common_pb";
import { STATUS } from "@/utils";

export const grpcGetLastChatpdfMessages = async (
  id: string
): Promise<Response<ChatpdfMessage.AsObject[]>> => {
...
export const grpcCreateChatpdfMessage = async (
  chatpdfId: string,
  message: string,
  role: GPTRole
...
export const grpcClearChatpdfMessage = async (
  id: string
): Promise<Response<GenericRes.AsObject>> => {
  return new Promise((resolve, reject) => {
    const req = new GenericReq();
    req.setId(id);
    chatpdfMessageService.clearChatpdfMessage(req, (err, res) => {
      if (err) {
        resolve({
          message: err.details,
          status: err.code,
